...
**Prompt/Ask:** Simplify the examples, separate the checkbox repro from the combo-box control-group example, and keep best-practice event semantics for each component.

## Completed
- Split the first demo into two separate examples: a simple combo-box control-group example and a dedicated checkbox repro.
- Simplified both `apps/boxes-web` and `apps/boxes-angular` so each repro now has its own isolated form and JSON output.
- Kept `boxes-select-multiple` on the select-like best-practice event model by restoring both `input` and `change`.
...
- The current simplified demo set is now:
  - combo-box control group that works as expected
...
## Files Changed
- `apps/boxes-web/index.html` - Split the demos into separate combo-box, checkbox, multi-select, and calendar sections with rationale copy.
...
- `libs/boxes/src/select-multiple/select-multiple.ts` - Restored native-like `input` + `change` dispatch for the select-like multi-select control.
...
- Browser verification on `apps/boxes-web`:
  - combo-box: selecting `Lemon` produced `{ "produce": "lemon" }`
...
- Browser verification on `apps/boxes-angular`:
  - combo-box: Angular and native both showed `{ "produce": "lemon" }`
...
## Next Steps
- Decide whether the deck should add a small “working control-group” slide for the combo-box or keep it as an app-only baseline.
