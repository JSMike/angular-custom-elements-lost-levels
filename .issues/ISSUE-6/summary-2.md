...
- Rebuilt the library and confirmed `.map` files are now emitted for `combo-box.js`, `checkbox.js`, and the shared Lit chunk.
...
- Parsed `dist/libs/boxes/combo-box.js.map` and confirmed its `sources` include `../../../libs/boxes/src/combo-box/combo-box.ts`.
